...
class Excursion(Model, ClusterFetcherMixin, IEvent, ICanAcceptModelVisitors, metaclass=CombinedModelAndABCMeta):
    """Model representing an excursion event in the system.

    :ivar id: UUID primary key field for the excursion
    :ivar _year: Integer field for the year of the excursion
    :ivar _month: Integer field for the month of the excursion
    :ivar _day: Integer field for the day of the excursion
    :ivar _hour: Integer field for the hour of the excursion
    :ivar _minute: Integer field for the minute of the excursion
    :ivar max_count_of_buyings: Maximum number of attendees allowed
    :ivar price: Price of the excursion
    :ivar address: Text field containing the excursion location
    ...
    async def accept(cls, visitor: IModelVisitor) -> None:
        """Accepts a model visitor and triggers the excursion visit method.
    ...
    async def delete(self, using_db: BaseDBAsyncClient | None = None) -> None:
        """Deletes the excursion and clears all associated user profiles.
    ...
    def date(self) -> datetime:
        """Gets the complete datetime of the excursion.
        
        :return: Datetime object representing the excursion's date and time
        """
        return datetime(self._year, self._month, self._day, self._hour, self._minute)

    @date.setter
    def date(self, value: datetime) -> None:
        """Sets the date and time components of the excursion.
    ...
    async def get_outdated_events(cls, date: datetime) -> list["Excursion"]:
        """Retrieves all excursions that have already occurred before the given date.
        
        :param date: The reference date to compare against
        :return: List of outdated Excursion instances
        """
    ...
    def get_pay_text_args(self) -> list[Any]:
        ...
        :return: List containing the excursion's date and address
        """
        return [self.date, self.address]

    async def get_count_of_buyings(self) -> int:
        """Returns the count of users that have purchased this excursion.
        
        :return: Number of users who have bought this excursion
        """
        return await self.bot_user_profiles.all().count()

    @classmethod
    async def create_event(cls, event_data: EventData) -> "Excursion":
        """Creates a new excursion event from the provided data.
        
        :param event_data: EventData object containing the excursion details
        :return: The newly created Excursion instance
        """
    ...
    async def get_by_id(cls, event_id: str) -> Optional["Excursion"]:
        """Retrieves an excursion by its ID.
        
        :param event_id: The UUID of the excursion to retrieve
        :return: The Excursion instance if found, None otherwise
        """
    ...
    async def get_cluster_and_count_of_clusters_by_date(cls, date: datetime, cluster_index: int, cluster_size: int
        ...
        """Retrieves a cluster of excursions and the total number of clusters.
        
        :param date: The reference date for filtering excursions
        :param cluster_index: The index of the cluster to retrieve
        :param cluster_size: The size of each cluster
        :return: Tuple containing the cluster of excursions and total cluster count
        """
